...
class _CalculatorFormState extends State<CalculatorForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("IPv4 Subnetting Calculator")),
      body: Column(
          children: [
            Container(
  ...
  _validateIPAddress(value) {
    ...
    if (arrOctet.length != 4) { // Make sure the ip address consist of 4 parts
      return false;
    }
    for (int i = 0; i < arrOctet.length; i++) {
      if (arrOctet[i].length > 3) { // To check if every octet have valid length of number
        return false;
      }
    }
    return true;
